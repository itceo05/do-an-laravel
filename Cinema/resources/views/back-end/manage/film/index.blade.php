            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <a href="{{route('film.create')}}" class="btn btn-gradient-primary btn-icon-text">
                                    <i class="mdi mdi-television-guide btn-icon-prepend"></i> AddData </a>
                            </div>
                            <div class="col-md-6">
                                <div class="row">
                        ...
                        <table class="table mt-3">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @foreach($CateOfFilm as $Cate)
                                        @if($Cate->film_id == $value->id)
                                        <pre>{{$Cate->name}}</pre>
                                        @endif
                                        @endforeach
                                    </td>
                                    <td><label
                                    class="{{($value->status == 2)? 'badge-outline-success' : (($value->status == 1)? 'badge-outline-info' : 'badge-outline-danger')}} badge  badge-pill">
                                    {{($value->status == 1)?  'Coming soon' : (($value->status == 2)? 'Now Showing' : 'Stop Showing')}}</label>
                                    </td>
                                    <td><a href="{{route('film.edit', $value->id)}}"
                        ...
                        <tr>
                            <td>{{$loop->index + 1}}</td>
                            <td class="tb-content">{{$value->name}}</td>
                            <td class="tb-content">{{$value->director}}</td>
                            <td><img src="{{url('Uploads')}}/{{$value->image}}" class="ImaPro" alt=""></td>
                            <td>{{$value->time}} Minutes</td>
                            <td> {{$value->release_date}}</td>
                            <td>
                            ...
                            <td><a href="{{route('film.edit', $value->id)}}" class="btn btn-gradient-dark btn-sm w-75">
                        ...
                        <tr class="odd">
                            <td valign="top" colspan="8" class="dataTables_empty text-center ">No results found for
                                keyword <strong> " {{$keywords}} "</strong><br><a href="{{route('film.index')}}"
