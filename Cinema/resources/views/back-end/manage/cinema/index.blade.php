            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <a href="{{route('cinema.create')}}" class="btn btn-gradient-primary btn-icon-text">
                                    <i class="mdi mdi-map-marker-radius btn-icon-prepend"></i> AddData </a>
                            </div>
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <div class="col-sm-12 col-md-10">
                                        <form action="{{route('search-cinema')}}" method="GET">
                        ...
                        <table class="table mt-3">
                            ...
                            <tbody>
                                ...
                                <tr class="text-center">
                                    <td>{{$loop->index + 1}}</td>
                                    <td>{{$value->name}}</td>
                                    <td class="tb-content">{{$value->address}}</td>
                                    <td>{{$value->quantity_Room}} Room</td>
                                    <td><label
                                            class="{{($value->status == 1)? 'badge-outline-success' : 'badge-outline-danger'}} badge  badge-pill">{{($value->status == 1)? 'In progress' : 'Pending'}}</label>
                                    </td>
                                    <td><a href="{{route('cinema.edit', $value->id)}}"
                                            class="btn btn-gradient-dark btn-sm w-75"> <i class="far fa-edit"></i></a>
                                        <form action="{{route('cinema.destroy', $value->id)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                        ...
                        <tr class="odd">
                            <td valign="top" colspan="8" class="dataTables_empty text-center ">No results found for
                                keyword <strong> " {{$keywords}} "</strong><br><a href="{{route('cinema.index')}}"
